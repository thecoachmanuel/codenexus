...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      title="Copy message"
      className="opacity-0 group-hover:opacity-100 flex h-6 w-6 items-center justify-center rounded-md bg-white/5 text-white/40 hover:bg-white/10 hover:text-white/80 transition-all duration-150"
    >
      {copied ? (
...
interface MessageBubbleProps {
  ...
  user: { imageUrl?: string | null; name?: string | null } | null | undefined;
}
      ...
      {msg.role === "user" ? (
        ...
        <div className="group flex items-start justify-end gap-2">
          <div className="max-w-[85%] space-y-1.5">
            ...
            <div className="flex items-end gap-1.5 justify-end">
              {/* Copy icon — appears on hover, left of bubble */}
        ...
        <div className="group flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            {/* Copy icon — below bubble, appears on hover */}
            {!isLiveStream && msg.content && (
...
export function ChatPanel({
  ...
  // Suppress unused var lint warnings — these are intentionally in props
  void userId;
  void workspaceId;
