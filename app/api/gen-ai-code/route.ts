...
11. **TOKEN LIMIT SURVIVAL (CRITICAL)**: You have a strict 8192 output token limit. To avoid crashing, you MUST be surgically precise. ONLY output the 1 or 2 files that absolutely need to change. NEVER output unchanged files. If a file is massive, extract your new feature into a new smaller component file and just add a 1-line import to the massive file. This prevents the system from truncating your output!
...
19. **PRECISION TARGETING**: When asked to fix or add a feature, DO NOT rewrite the entire app architecture. Understand exactly what component handles that feature, and ONLY output that specific component file. The rest of the app is safely preserved in memory.
`;
