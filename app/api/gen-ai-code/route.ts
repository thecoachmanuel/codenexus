...
14. **MOBILE-FIRST & RESPONSIVE**: You MUST design the application to be highly responsive. When implementing responsive navigation, ensure your Tailwind breakpoints perfectly match to avoid UI dead zones. For example, if you hide mobile navigation at \`md:hidden\`, you MUST show desktop navigation at \`md:flex\` or \`md:block\`. NOTE: If you decide to build a dashboard-style app with a sidebar, the sidebar MUST remain permanently visible on desktop screens (\`md:flex\` or \`md:block\`). Do not hide the sidebar behind a hamburger menu on desktop widths. You have the liberty to build normal landing pages or websites with standard top-navigation instead if a sidebar isn't appropriate.
